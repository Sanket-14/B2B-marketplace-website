...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT user_id, email, password, comp_name FROM userdata WHERE comp_name = ?");
        ...
        if (password_verify($password, $row['password'])) {
            ...
            if ($check_result->num_rows > 0) {
                ...
                if (isset($_POST["remember_me"])) {
                    ...
                    $update_stmt = $conn->prepare("UPDATE userdata SET rm_token = ? WHERE comp_name = ?");
                    $update_stmt->bind_param("ss", $token, $row["comp_name"]);
                    $update_stmt->execute();
                    $update_stmt->close();
                }
            ...
            // Incorrect password
            $error = "comp_name address or password entered is incorrect. Please check your comp_name address or password.";
        }
    } else {
        // User not found
        $error = "comp_name address or password entered is incorrect or not registered. Please check your comp_name address or create a new user using this comp_name!";
    }
    ...
    $stmt->close();
            ...
            $error="Email is not an admin email";
        }
}
       ...
       <div class="col-md-6 right-box">
          <div class="row align-items-center">
                ...
                <div class="input-group mb-3">
                    <input type="email" class="form-control form-control-lg bg-light fs-6" id="email" name="email" placeholder="Admin Email" required>
                </div>
                
                <div class="input-group mb-3">
                    <input type="comp_name" class="form-control form-control-lg bg-light fs-6" id="comp_name" name="comp_name" placeholder="company name" required>
                </div>
                ...
                <div class="input-group mb-2">
                    ...
                    <label class="form-check-label p-1" for="remember_me">Remember Me</label>
                </div>
