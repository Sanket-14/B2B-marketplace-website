...
if (isset($_COOKIE["buyer_remember_token"])) {
    ...
    $stmt = $conn->prepare("SELECT buyer_email, buyer_comp_name FROM buyer_userdata WHERE buyer_rm_token = ?");
    $stmt->bind_param("s", $buyer_remember_token);
    $stmt->execute();
    ...
    if ($result->num_rows == 1) {
        ...
        $_SESSION["buyer_comp_name"] = $row["buyer_comp_name"];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT  buyer_email, buyer_password, buyer_comp_name FROM buyer_userdata WHERE buyer_email = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $row['buyer_password'])) {
            ...
            $comp_name = $row["buyer_comp_name"];

            // Initialize $check_stmt
            $check_stmt = $conn->prepare("SELECT buyer_comp_name FROM buyer_company_data WHERE buyer_comp_name = ?");
            ...
            if ($check_result->num_rows > 0) {
                ...
                if (isset($_POST["remember_me"])) {
                    ...
                    setcookie("buyer_remember_token", $token, time() + (86400 * 30), "/"); // Cookie valid for 30 days
                    
                    // Save the token to the database or session for future validation if needed
                    // Example: $_SESSION["remember_token"] = $token;
                    $update_stmt = $conn->prepare("UPDATE buyer_userdata SET buyer_rm_token = ? WHERE buyer_email = ?");
                    $update_stmt->bind_param("ss", $token, $row["buyer_email"]);
                    $update_stmt->execute();
                    $update_stmt->close();
                }
    ...
    <!-------------------- ------ Right Box ---------------------------->
       ...
       <div class="col-md-6 right-box">
          <div class="row align-items-center">
                ...
                <form method="post" action="buyer_login.php" class="login-form">
                ...
                <div class="input-group mb-3">
                    <button class="btn btn-lg btn-success w-100 fs-6" type="submit">Buyer Login</button>
                </div>
            </form>
                <!-- for google login -->
                <!-- <div class="input-group mb-3">
                ...
                <div class="row">
                    <small>Don't have account? <a href="./buyer_registration.php">Register</a></small>
                </div>
                <div class="row">
