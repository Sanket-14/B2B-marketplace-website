<?php
session_start();
if (basename($_SERVER['PHP_SELF']) !== 'success_content.php') {
    session_destroy();
}
require('dbcon.php');
$error = "";
...
// If userEmail is not set, show alert and then redirect to register.php
    ...
    if (empty($error)) {
        ...
        if ($emailResult->num_rows > 0) {
            ...
            if ($conn->query($sql) === TRUE) {
                echo '<script>window.location.href = "add_listing.php?companyName=' . $companyName . '";</script>';
...
<body>
    ...
    <form action="success_content.php" method="POST">
        ...
        <div class="form-group">
            ...
            <input style="background-color:lavender; border:none" type="email" id="email" name="email" value="<?php echo $userEmail; ?>" required readonly>
        </div>
